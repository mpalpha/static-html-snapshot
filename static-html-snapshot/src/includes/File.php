...
class File
{
    ...
    public function create_snapshot()
    {
        ...
        if (! is_writable(plugin_dir_path(__FILE__))) {
            $resType = 'Error';
            $resData = [ 'message' => 'Check Write permissions' ];
            $resCode = 500;
            $hasError = true;
        }
    ...
    private function generateStaticSite(string $name, array $permalinks = [])
    {
        ...
        $strip_query_command = 'cd '.$output_path.' && tar -cvf '.get_home_path().'/'.$name.'.tar '.$name;
        $create_tar_command = 'rm -rf '.$output_path;
        ...
        exec($wget_command, $output, $result);
        exec($move_command, $output, $result);
        ...
        exec($strip_query_command, $output, $result);
        // create tar file
        exec($create_tar_command, $output, $result);
        ...
        if ($return) {
            ...
            $resData = [
                ...
                'move_cmd' => $move_cmd,
                'strip_cmd' => $strip_query_command,
                'create_tar_cmd' => $create_tar_command
            ];
            $resCode = 500;
